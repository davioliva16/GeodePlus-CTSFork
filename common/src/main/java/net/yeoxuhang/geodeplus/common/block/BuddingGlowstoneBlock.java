...
public class BuddingGlowstoneBlock extends AmethystBlock {
    ...
    public List<ItemStack> getDrops(BlockState blockState, LootParams.Builder builder) {
        ...
        ItemStack nether = new ItemStack(GeodePlusBlocksRegistry.BUDDING_GLOWSTONE.get());
        ItemStack basalt = new ItemStack(GeodePlusBlocksRegistry.BUDDING_BASALT_GLOWSTONE.get());
        ItemStack blackstone = new ItemStack(GeodePlusBlocksRegistry.BUDDING_BLACKSTONE_GLOWSTONE.get());
        if (EnchantmentHelper.hasSilkTouch(pickaxe) && GeodePlusCommonConfigs.SHOULD_SILK_TOUCH_BUDDING_BLOCKS.get() && blockState.is(GeodePlusBlocksRegistry.BUDDING_GLOWSTONE.get())){
            return Collections.singletonList(nether);
        }
        if (EnchantmentHelper.hasSilkTouch(pickaxe) && GeodePlusCommonConfigs.SHOULD_SILK_TOUCH_BUDDING_BLOCKS.get() && blockState.is(GeodePlusBlocksRegistry.BUDDING_BASALT_GLOWSTONE.get())){
            return Collections.singletonList(basalt);
        }
        if (EnchantmentHelper.hasSilkTouch(pickaxe) && GeodePlusCommonConfigs.SHOULD_SILK_TOUCH_BUDDING_BLOCKS.get() && blockState.is(GeodePlusBlocksRegistry.BUDDING_BLACKSTONE_GLOWSTONE.get())){
