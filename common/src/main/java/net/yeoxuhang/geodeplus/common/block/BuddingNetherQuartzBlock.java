...
public class BuddingNetherQuartzBlock extends AmethystBlock {
    ...
    public List<ItemStack> getDrops(BlockState blockState, LootParams.Builder builder) {
        ...
        ItemStack nether = new ItemStack(GeodePlusBlocksRegistry.BUDDING_NETHER_QUARTZ.get());
        ItemStack basalt = new ItemStack(GeodePlusBlocksRegistry.BUDDING_BASALT_QUARTZ.get());
        ItemStack blackstone = new ItemStack(GeodePlusBlocksRegistry.BUDDING_BLACKSTONE_QUARTZ.get());
        if (EnchantmentHelper.hasSilkTouch(pickaxe) && GeodePlusCommonConfigs.SHOULD_SILK_TOUCH_BUDDING_BLOCKS.get() && blockState.is(GeodePlusBlocksRegistry.BUDDING_NETHER_QUARTZ.get())){
            return Collections.singletonList(nether);
        }
        if (EnchantmentHelper.hasSilkTouch(pickaxe) && GeodePlusCommonConfigs.SHOULD_SILK_TOUCH_BUDDING_BLOCKS.get() && blockState.is(GeodePlusBlocksRegistry.BUDDING_BASALT_QUARTZ.get())){
            return Collections.singletonList(basalt);
        }
        if (EnchantmentHelper.hasSilkTouch(pickaxe) && GeodePlusCommonConfigs.SHOULD_SILK_TOUCH_BUDDING_BLOCKS.get() && blockState.is(GeodePlusBlocksRegistry.BUDDING_BLACKSTONE_QUARTZ.get())){
